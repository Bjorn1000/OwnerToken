...
contract('OwnerToken', function(accounts) {
    ...
    it('initializes the contract with the correct values', function() {
        return OwnerToken.deployed().then(function(instance) {
            ...
            assert.equal(name, 'OwnerToken', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function(symbol) {
            assert.equal(symbol, 'OTN', 'has correct symbol');
            return tokenInstance.version();
        }).then(function(version) {
            assert.equal(version, 'v0.01', 'is correct version')
        });
    });
    ...
    it('transfers token ownership', function() {
      ...
      it('handles delegated token transfers', function () {
        return OwnerToken.deployed().then(function(instance) {
            ...
            // Try transferring someething larger than the sender's balance
